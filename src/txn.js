function update_balance(account, amount, operation) {
  if (operation === "debit") {
...
export function Update_Account_Balance_On_Txn(accounts, setAccounts, txn, isDelete = false) {
  ...
  switch (txn.type) {
    case 1: {
      ...
      allAccounts[idx] = update_balance(allAccounts[idx], txn.amount, isDelete ? "credit" : "debit")
    } break;
    case 2: {
      ...
      allAccounts[idx1] = update_balance(allAccounts[idx1], txn.amount, isDelete ? "credit" : "debit")
      allAccounts[idx2] = update_balance(allAccounts[idx2], txn.amount, isDelete ? "debit" : "credit")
    } break;
    case 3: {
      let idx = allAccounts.findIndex(val => val.id === txn.account.id);
      if (idx === -1) return false;
      allAccounts[idx] = update_balance(allAccounts[idx], txn.amount, isDelete ? "debit" : "credit")
    } break;
    default: break;
  }
...
export function Delete_Account_And_Related_Txn(accounts, setAccounts, transactions, setTransactions, accId) {
...
export function Delete_Txn(accounts, setAccounts, transactions, setTransactions, txnId) {
  let allTxns = transactions
  let idx = allTxns.findIndex(val => val.id === txnId);
  Update_Account_Balance_On_Txn(accounts, setAccounts, allTxns[idx], true)
  const newTxns = transactions.filter(val => val.id !== txnId)
  setTransactions(newTxns)
  return true;
}